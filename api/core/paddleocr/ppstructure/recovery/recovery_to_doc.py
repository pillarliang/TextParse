...
def convert_info_markdown(layout_boxes, save_folder, img_floader):
    # 1. 识别标题信息
    ...
    for i, region in enumerate(layout_boxes):
        ...
        if region["type"].lower() == "figure":
            img_path = os.path.join(
                img_floader, "{}_{}.jpg".format(region["bbox"], img_idx)
            )
            ...
            if i > 0 and layout_boxes[i - 1]["type"].lower() == "figure_caption":
                ...
                res += f"![{figure_caption}]({str(img_path)})\n\n"  # 插入图片
            elif (
                i + 1 < len(layout_boxes)
                and layout_boxes[i + 1]["type"].lower() == "figure_caption"
            ):
                figure_caption = get_region_text(layout_boxes[i + 1])
                res += f"![{figure_caption}]({str(img_path)})\n"  # 插入图片
                res += f"{figure_caption}\n\n"  # 插入描述
            ...
            table_content = re.search(
                r"(<table.*?</table>)", region["res"]["html"], re.DOTALL
            ).group(1)
            res += f"{table_content}\n\n"
